...
shaka.media.StreamingEngine = class {
  ...
  async initSourceBuffer_(mediaState, reference) {
    ...
    if (timestampOffset != mediaState.lastTimestampOffset ||
      ...
      if (mediaSourceEngine.needsReload(mediaState.type, codecs)) {
        if (mediaState.type === ContentType.VIDEO) {
      ...
      try {
        ...
        await mediaSourceEngine.setStreamProperties(
            ...
            appendWindowEnd, mediaState.stream.codecs,
            mediaState.stream.mimeType, forceTransmuxTS);
      } catch (error) {
 ...
 *   The previous codecs reported on the source buffer
